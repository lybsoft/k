...
if [ "$gaming" = "1" ]; then
echo is GAMING
	BDIR=${RDIR}/build_1$isg5
	gamingstr=_gaming
  else
  echo is not GAMING
	BDIR=${RDIR}/build$isg5
fi
...
CLEAN_DIR() {
	...
	rm -rf $DDIR
	rm -f $RDIR/$OUTDIR/${DEVICE}_${VER}-v20bcustom.zip
}
...
ZIP_UP() {
	...
	cd $DDIR
	zip -7qr $RDIR/$OUTDIR/${DEVICE}_${VER}-lineage$gamingstr$ocstr.zip * \
